      ...
      <div class="cart-body" v-for="(cart,index) in shopCartList" :key="index">
        <ul class="cart-list" v-for="cartInfo in cart.cartInfoList" :key="cartInfo.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="cartInfo.isChecked" @click="changeOneIsCheck(cartInfo)">
          </li>
...
export default {
  ...
  methods: {
    ...
    async changeOneIsCheck(cartInfo) {
      try {
        await this.$store.dispatch('changeOneIsCheck', { skuId: cartInfo.skuId, isChecked: cartInfo.isChecked ? 0 : 1 })
        alert('状态修改成功');
        this.getShopCartList();
      } catch (error) {
        alert('状态修改失败');
    ...
    async deleteOneCart(cartInfo) {
      try {
        await this.$store.dispatch('changeOneCart', cartInfo.skuId);
        alert('删除成功');
        this.getShopCartList();
      } catch (err) {
        alert('删除失败')
    ...
    async deleteAllCart() {
      ...
      try {
        await this.$store.dispatch('changeAllCart', skuIdList);
        alert('删除成功');
        this.getShopCartList();
      } catch (error) {
  ...
  computed: {
    ...
    isAllCheck: {
      ...
      async set(value) {
        ...
        this.shopCartList.forEach(item => {
          item.cartInfoList.forEach(item1 => {
            if (item1.isChecked !== isChecked) skuIdList.push(item1.skuId);
          })
        })
        try {
          await this.$store.dispatch('changeAllIsChecked', { isChecked, skuIdList });
          alert('改变状态成功');
          this.getShopCartList();
        } catch (error) {
          alert('改变状态失败')
