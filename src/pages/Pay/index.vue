    ...
    async pay() {
      try {
        ...
        this.$alert(`<img src="${imgUrl}">`, '请使用微信扫码支付', {
          ...
          showCancelButton: true, // 返回按钮
          cancelButtonText: '支付遇到问题', //返回按钮文本
          confirmButtonText: '我已成功支付',
          center: true,
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              // if (this.payState === 0) {
              //   this.$message.error('赶紧掏钱');
              // }
        ...
        this.$message.error("二维码获取失败" + error.message)
      }
