<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address, index) in userAddressList" :key="index">
        <span class="username" :class="{ selected: address.isDefault === '1' }" @click="changeDefault(userAddressList, address)">{{ address.consignee }}</span>
        <p>
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(detail, index) in tradeInfo.detailArrayList" :key="index">
          <li>
            <img :src="detail.imgUrl" style="width: 80px; height: 80px" alt="" />
          </li>
          <li>
            <p>
              {{ detail.skuName }}
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      const res = await this.$API.reqSubmitOrder(tradeNo, tradeInfo);
      if (res.code === 200) {
        alert('订单创建成功，将为您跳转到支付页面')
