          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd changepirce="0" :class="{'active':saleAttrValue.isChecked==='1' }" @click="changeIsChecked(saleAttr.spuSaleAttrValueList,saleAttrValue)" v-for="saleAttrValue in saleAttr.spuSaleAttrValueList" :key="saleAttrValue.id">{{saleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
export default {
  ...
  methods: {
    ...
    async addShopCart() {
      try {
        await this.$store.dispatch('AddOrUpdateToCart', { skuId: this.skuId, skuNum: this.skuNum });
        alert('添加购物车成功');
        // 将商品信息保存到sessiionstorege中
